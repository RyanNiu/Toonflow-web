<template>
  <!-- 嵌入模式：独立页全屏工作台，无弹窗 -->
  <div v-if="embedded" class="storyboard-embedded">
    <div class="content pr">
      <div class="rightPanel">
        <div class="panelTabs">
          <button type="button" class="panelTab" :class="{ active: rightTab === 'fragment' }" @click="rightTab = 'fragment'">
            <i-file-text theme="outline" :size="16" /> 片段
          </button>
          <button type="button" class="panelTab" :class="{ active: rightTab === 'storyboard' }" @click="rightTab = 'storyboard'">
            <i-pic theme="outline" :size="16" /> 分镜图
          </button>
        </div>
        <div class="panelContent">
          <div v-show="rightTab === 'fragment'" class="fragmentPlaceholder">
            <div class="fragmentHeader">
              <i-file-text theme="outline" :size="18" /> 片段内容
            </div>
            <div class="fragmentBody">
              <template v-if="segmentList.length">
                <div v-for="seg in segmentList" :key="seg.index" class="fragmentItem">
                  <div class="fragmentItemTitle">片段 {{ seg.index }}</div>
                  <div class="fragmentItemDesc">{{ seg.description }}</div>
                  <div v-if="seg.emotion || seg.action" class="fragmentItemMeta">
                    <span v-if="seg.emotion">情绪：{{ seg.emotion }}</span>
                    <span v-if="seg.action">动作：{{ seg.action }}</span>
                  </div>
                </div>
              </template>
              <div v-else class="fragmentEmpty">对话生成片段后将显示在此处</div>
            </div>
          </div>
          <draggableCanvas v-show="rightTab === 'storyboard'" v-model="gridData" :generatingIds="generatingIds" @replaceShot="replaceShot" @generateImage="handleGenerateImage" />
        </div>
      </div>
      <div class="chatBox pa" :class="{ hoverState: gridData.length > 0 }">
        ...
        <div class="f ac jb">
          ...
          <div class="chatBox-actions">
            <a-button class="btn" danger @click="handleDeleteAll">删除</a-button>
    ...
    <detectionImage
      ...
      v-model:modalShow="detectionModalShow"
      @save="$emit('save')" />
  </div>
  <!-- 弹窗模式：原有 Modal -->
  <a-modal
...
// 嵌入模式下 detectionImage 仍需要控制自己的弹窗，用独立变量
const detectionModalShow = ref(false);
...
const history = ref<ChatMessage[]>([
  {
    ...
    data: [
      {
        type: "text",
        text: "欢迎使用Robou！我已经收到你的剧本与相关资产,请和我说“开始”启动生成分镜图的制作吧！",
      },
    ],
  },
]);
...
function handleWsMessage(msgData: { type: string; data: any }) {
  ...
  const messageHandlers: Record<string, (data: any) => void> = {
    ...
    // 片段数据更新 - 保存到「片段内容」框体展示
    segmentsUpdated: (data) => {
      const list = data?.data;
      segmentList.value = Array.isArray(list) ? list : [];
    },
...
// 清空历史记录（同时清空片段与分镜，重新开始制作时会用新内容替换）
...
function handleDeleteAll() {
  Modal.confirm({
    title: "确认删除",
    content: "确定要删除当前片段和分镜图吗？删除后不可恢复。",
    okText: "确定",
    cancelText: "取消",
...
// 进入页面时建立连接，以便收到后端恢复的片段/分镜（切换 Tab 再回来时能显示片段内容）
onMounted(() => {
...
const rightTab = ref<"fragment" | "storyboard">("fragment");

/** 对话生成的片段列表，展示在「片段内容」框体 */
...
async function exportAll() {
  ...
  try {
    ...
    if (!props.embedded) modalShow.value = true;
...
.content {
  ...
  .panelTab {
    ...
    &.active {
      color: var(--color-text);
      background: var(--color-accent, #fffbe6);
