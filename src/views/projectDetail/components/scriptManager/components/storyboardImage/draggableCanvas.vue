    ...
    <div class="canvas" ref="canvasRef" :style="canvasStyle">
      <div
        ...
        <div class="topMenu fc" @mousedown.stop="handleGridMouseDown($event, grid)">
          ...
          <div class="assetsTagsRow">
            <span v-for="(sub, index) in grid.assetsTags" :key="index" class="assetTag">{{ sub.text }}</span>
          </div>
        </div>
        ...
        <a-spin :spinning="isGenerating(grid.id)" tip="生成中...">
          <div class="grid" :style="getGridStyle(grid.cells.length)">
            <div v-for="(cell, index) in grid.cells" :key="index" class="gridItem" :style="getImageSize">
              <div class="tag">镜头{{ grid.segmentId }}-{{ index + 1 }}</div>
...
.viewport {
  ...
  background-image: linear-gradient(var(--color-border, #eee) 1px, transparent 1px),
    linear-gradient(90deg, var(--color-border, #eee) 1px, transparent 1px);
...
.gridContainer {
  ...
  &:hover {
    box-shadow: 0 4px 12px $shadowMedium;
  }
