<template>
  <div class="video-config-form">
    ...
    <template v-if="localConfig.mode === 'multi'">
      <div class="form-row">
        ...
        <div class="multi-images">
          ...
          <div
            v-if="editable && (!localConfig.images || localConfig.images.length < maxImagesForCurrentMode)"
            class="add-image-box"
      ...
      <div class="form-row" v-if="editable">
        <label></label>
        <span class="tip">
          拖拽调整顺序 | {{ localConfig.images?.length || 0 }}/{{ maxImagesForCurrentMode }}张
        </span>
      </div>
    </template>
    ...
    <ImageSelector
      ...
      :script-id="scriptId"
      :mode="selectorMode"
      :max-images="maxImagesForCurrentMode"
...
// 当前模式下允许的最大图片数（防止多图模式被错误限制为 1 张）
const maxImagesForCurrentMode = computed(() => {
  const base = getMaxImages(localConfig.manufacturer, localConfig.model) || 1;
  if (localConfig.mode === "multi") return Math.max(base, 9);
  if (localConfig.mode === "startEnd") return Math.max(base, 2);
  if (localConfig.mode === "single") return 1;
  return base;
});
...
function onModeChange() {
  ...
  if (localConfig.mode == "text") {
    ...
    if (localConfig.images.length && maxImagesForCurrentMode.value > 0) {
      localConfig.images = localConfig.images.slice(0, maxImagesForCurrentMode.value);
    }
...
onMounted(async () => {
  ...
  if (!localConfig.model) {
    ...
    // 必须同时匹配 manufacturer 和 model，否则同厂商多模型（如 zzgf 下 sora 与 veo3.1-components）会误匹配到第一项
    const selectedManufacturer = manufacturerList.value.find(
      (i) => i.manufacturer === localConfig.manufacturer && i.model === localConfig.model,
    );
